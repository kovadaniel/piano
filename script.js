import {Keyboard, SynthPanel, OscillatorPanel, EffectsPanel, SequencerPanel, InstrumentsPanel} from './modules/controls.js';
...
/*
    ...
    in our SynthPanel controller we initiate each synthesizer as:
        `new Tone.${synthName}(${params})` // although params exist only for paino, all other case they are: ''
...
*/
    ...
    if(action.type == 'playKey'){
        ...
        if(!state.playingNotes.has(note)){ // if it is not repeat of the same event by the clamped key
            ...
            */
           /*
            if(saveSynths.includes(state.synth.type)){ // && !Object.keys(state.effects).includes('BitCrusher'))
                console.log("[Play key] set oscillator:", state.oscillator);
                state.synth.object.set({envelope: {oscillator: state.oscillator}});
            }
            */
                //state.synth.object.oscillator.type = state.oscillator;

            // 3. start playing a note:
            //state.synth.object.triggerAttack(note);
            // //Tone.start();

            // 3. start playing a note (with PolySynth):
    ...
    if(action.type == 'stopKey'){
        ...
        console.log("state.playingNotes.size", state.playingNotes.size);
        ...
        if(action.synthType !== state.synth.type){
            ...
            if(action.oscillator){
                ...
                for(let i=0; i < oscParentDOM.children.length; i++){
                    ...
                    console.log("in osc handling in changeSynth. action:", action);
            ...
            return Object.assign({}, state, {synth: {type: action.synthType, object: action.object}, 
    ...
    if(action.type == 'changeOscillator'){
        if(state.oscillator !== action.oscillator){
            ...
            console.log('[changeOscillator] params of the synth:', state.synth.object.get());
    ...
    if(action.type == 'turnEffect'){
        ...
        */
        ...
        if(effects[action.name]){ // If this effect is already ON
            ...
            return Object.assign({}, state, {effects: effects});
        } else{
            // connecting effect
    ...
    if(action.type == 'adjustEffect'){
        if(state.effects[action.name]){
            //console.log('evaluating setting:', `state.effects[action.name]${action.reference[0]+action.value+action.reference[1]}`);
    ...
    if(action.type == "turnSequencerNote"){
        ...
        console.log("currentCell", currentCell, "coordinates:", action.column, action.row);
    ...
    function restartTransport(seq){
        ...
        const repeat = (time) => {
            ...
            seq.grid[beat].forEach(cell => {
                ...
                if(!seq.fixed)
                    seq.synth = state.synth.object; // doesn't work after the first time (1-st time does work)
                if (cell.isActive) {
                    seq.synth.triggerAttackRelease(cell.note, "8n", time);
                }
            });
    ...
    if(action.type == 'setSequencerSynth'){
        // 0. copy! our sequencer from our state for further manipulations.
        let seq = Object.assign({}, state.sequencer);
        // 1. if the synth in the sequencer is not fixed
        if(!seq.fixed){
            // 2. take synth from piano 
            seq.synth = state.synth.object;
        } 
        // 3. set 'fixed' if it wasn't or set 'not fixed' if it was
        seq.fixed = !seq.fixed;
...
async function runApp(){
    ...
    let state = {playingNotes: new Set(),
                 ...
                 synth: {type: 'Synth', object: baseSynth},
                 miniKeyboard: true,
