...
export class Keyboard{
    constructor(state, dispatch){
        ...
        this.dom.addEventListener('touchmove', (e) => {
            ...
            //console.log('[touchmove] key', key);

            // make sure a key was found
            if(key.classList.contains('key')){
                const note = key.dataset.note;
                /*
                if(this.touchedKey === undefined){ // first moving
                    dispatch({type: 'playKey', note: note, dom: key});
                    this.touchedKey = key;
                } else */
                if(this.touchedKey == key){
                    dispatch({type: 'playKey', note: note, dom: key});
                } else if(this.touchedKey != key){
                    dispatch({type: 'stopKey', note: this.touchedKey.dataset.note, dom: this.touchedKey});
                    this.touchedKey = key;
                }
            }
        })
    ...
    makeKey(note, index) {
        ...
        const dom = elt('li', 
            ...
            onmouseleave: (e) => {
                if(e.buttons === 1){
                this.dispatch({type: 'stopKey', note: note, dom: dom})
            }},
            onmouseenter: (e) => {
                if(e.buttons === 1){
                    this.dispatch({type: 'playKey', note: note, dom: dom})
                }},
            ...
            ontouchend: (e) => {
                //e.preventDefault();
                this.dispatch({type: 'stopKey', note: note, dom: dom});
                if(this.touchedKey) this.dispatch({type: 'stopKey', note: this.touchedKey.dataset.note, dom: this.touchedKey});
            },
...
export class SynthPanel{
    constructor(state, dispatch, tools){  // synth is an object with {type: ..., object:...,}
        this.synths = {};
        /*
        tools.synths = ['Synth': SynthObject, 'MonoSynth': MonoSynthObject, 'DuoSynth': DuoSynthObject, 
            ...
            ...Object.keys(this.synths).map(synth => {
                ...
                return elt('div', 
                        ...
                        onclick: () => dispatch({type: 'changeSynth', 
                                                 ...
                                                 // patternParams are the params that are needed when synth has 
                                                 // been evaluating from his pattern (needed for fixation of synth in sequencer)
                                                 patternParams: tools.synths[synth],
                                                 })},
...
export class EffectsPanel{
    constructor(state, dispatch, tools){
        ...
        */
        ...
        /* effects are evaluated when they are first selected. Such an 
         * approach lets us avoid background noises at the start. It looks like:
                this.effects == { Chorus: evaluated-string-"new Tone.Chorus().toDestination().start()", ...} 
         */
        this.effects = {};
                        ...
                        elt('div', {
                            ...
                            onclick: (e) => {
                                ...
                                dispatch({type: 'turnEffect', 
                                            ...
                                            dom: e.currentTarget,
                                             // pattern is needed for a fixation of a synth in sequencer when lock is pressed
                                             // pattern: <pattern-from-tools.effects-for-creating-this-effect-by-eval()> 
                                            pattern: tools.effects.filter(e => e.name === effect.name)[0].pattern,
                                        })
                                }},
...
export function collectEffectParams(name){
    /*
    ...
    constructor(state, dispatch, tools){
        ...
        this.dom = elt('div', {className: 'sequencer-container'},
            ...
            elt('div', {className: 'lock-container'},
                elt('input', {type: 'checkbox', id:'inputLock', onchange: () => dispatch({type: 'setSequencerSynth', dom: this.playIcon.parentNode})}), // parentNode is a play-button
                this.lockSVG,
...
export class InstrumentsPanel{
    constructor(state, dispatch, tools){
        this.baseParams = tools.sequencer;
        /*
         * this section is for the volume of the piano (not only for sequencer)
        this.volume = elt('form', {id: 'sequencer-volume-form'},
            elt('input', {id: 'sequencer-volume', 
                type: 'range', 
                max: this.baseParams.volume,
